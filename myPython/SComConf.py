...
def do_tx():
	print "Start Tx now..."
	print "COM port: %s" % port.get()
	print "Baudrate: %s" % baudrate.get()
...
#rootWin.geometry('400x200')

pw1 = PanedWindow(rootWin) 
...
pw1.add(frameInput)
port = inputCellE(frameInput, "Port:", "COM1")
baudrate = inputCellE(frameInput, "Baudrate:", "19200")
#FIVEBITS(5), SIXBITS(6), SEVENBITS(7), EIGHTBITS(8)
bytesize = inputCellE(frameInput, "DataBits:", "8") 
#PARITY_NONE(N), PARITY_EVEN(E), PARITY_ODD(O), PARITY_MARK(M), PARITY_SPACE(S)
parity = inputCellE(frameInput, "Parity:", "N")
#STOPBITS_ONE(1), STOPBITS_ONE_POINT_FIVE(1.5), STOPBITS_TWO(2)
stopbits = inputCellE(frameInput, "StopBits:", "1")
timeout = inputCellE(frameInput, "Timeout(s):", "10")
fc_items = ('NONE', 'XONXOFF', 'RTSCTS', 'DSRDTR')
flowctrl = inputCellOM(frameInput, "FlowCtrl:", fc_items)

pw2 = PanedWindow(pw1, orient=VERTICAL)
pw1.add(pw2)
frameTransmitted = LabelFrame(pw2, text="Transmitted", width=800, height=100)
entry_tx = Entry(frameTransmitted)
entry_tx.pack(side=LEFT, fill=X, expand=1)
...
text_recv = ScrolledText(frameReceived)
text_recv.pack(side=TOP, fill=BOTH, expand=1)
